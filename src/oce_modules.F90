...
CHARACTER(LEN=8)              :: which_NN = 'kjczrsiy'
REAL                          :: scaling_nnfluxes = 1. ! by how much the fluxes generated by GM neuralnet should be scaled
...
character(20)                  :: which_pgf='shchepetkin'
 ...
 NAMELIST /oce_dyn/ state_equation, C_d, A_ver, &
                    ...
                    use_GM_NN, which_NN, scaling_nnfluxes, &
...
REAL(KIND=WP), ALLOCATABLE, DIMENSION(:,:,:) :: nn_input_full
! Means and standard deviations needed to unnormalize outputs
REAL(KIND=WP), ALLOCATABLE, DIMENSION(:) :: means_train, stds_train
REAL(KIND=WP), ALLOCATABLE, DIMENSION(:) :: means_full, stds_full
! Temperature fluxes computed by GM neuralnet
REAL(KIND=WP), ALLOCATABLE, DIMENSION(:,:,:)       :: GM_temperature_flux

! Debugging: what is added in apply_gm_fluxes
REAL(KIND=WP), ALLOCATABLE, DIMENSION(:,:) :: ttf_diff
